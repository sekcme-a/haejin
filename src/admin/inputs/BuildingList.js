...
import DropperImage from "components/Common/DropperImage"

const BuildingList = ({type, item, text}) => {
  const {data, setData, handleData} = useData()
  const router = useRouter()

  const [values, setValues] = useState([{title:"", imgUrl:"", text:""}])
  ...
  const onDefaultClick = (index) => {
    if(index===0) handleImgUrl(index, "https://cdn.imweb.me/thumbnail/20210408/974c98b211dd2.png")
    if(index===1) handleImgUrl(index, "https://cdn.imweb.me/thumbnail/20210408/d4286c4d03bf3.png")
    if(index===2) handleImgUrl(index, "https://cdn.imweb.me/thumbnail/20210408/4b501db5cd25a.png")
    if(index===3) handleImgUrl(index, "https://cdn.imweb.me/thumbnail/20210408/ad79a598a5a50.png")
    if(index===4) handleImgUrl(index, "https://cdn.imweb.me/thumbnail/20210408/a9c2cd5770492.png")
    if(index===5) handleImgUrl(index, "https://cdn.imweb.me/thumbnail/20210408/c48f7d847354c.png")
  }
    ...
    <div>
      {
        values?.map((itm, index) => (
          <li key={`${itm.imgUrl}_${index}`} style={{marginBottom:"40px", listStyle:"none"}}>
            ...
            <DropperImage imgURL={itm.imgUrl} setImgURL={(url) => handleImgUrl(index, url)} path={`data/${type}/${item}_${index}`} setLoading={setIsImageLoading} />
            {index<=5 && <Button
              variant="contained"
