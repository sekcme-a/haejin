...
const One = () => {
  ...
  const [fetchedData, setFetchedData] = useState({
    text1:`(주)혜진종합관리은 공동주택관리 업계를 선도하고 있습니다.
  ...
  return(
    <div className={styles.main_container}>
      <motion.h2 {...initial(0,-50,0)} animate={{...start(0.5,0.2)}} style={{position:"relative", fontWeight:"normal"}}>
        {fetchedData.three_text1}
        <EditButton type="major" item="three_text1" text="텍스트"  />
      </motion.h2>
      ...
      <motion.h3 {...initial(0,50,0)} animate={{...start(1,1.0)}} style={{position:"relative", fontWeight:"normal"}}>
        {fetchedData.three_text2}
        <EditButton type="major" item="three_text2" text="텍스트"  />
      </motion.h3>
      <motion.div className={styles.image_container} ref={inViewRef} {...initial(0,0,50)} animate={ani1}>
        <img src={fetchedData.three_imgUrl} alt="부동산자산관리" />
        <EditButton type="major" item="three_imgUrl" text="이미지 삽입" mode="image" defaultImg="https://cdn.imweb.me/thumbnail/20220512/446fb2e11de64.jpg"/>
      </motion.div>
    </div>
  )
}
