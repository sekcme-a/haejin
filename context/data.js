...
export function DataProvider(props){
  ...
  const fetch_thumbnails_list = async (type) => {
    const query = await db.collection("team").doc("haejin").collection("posts").where("condition", "==", "게제중").where("type","==",type).orderBy("publishedAt", "desc").orderBy("savedAt", "desc").get()
